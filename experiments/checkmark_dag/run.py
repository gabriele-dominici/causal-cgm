...
from causal_cbm.utils import cace_score, explanation_to_latex, evaluate_exp, interventions_from_root, compute_pns_matrix
import numpy as np
import os
from dataset import checkmark_dataset
...
for i in range(n_seeds):
    ...
    x_train, s_train, dag_init = checkmark_dataset(n_samples, seed, 0.1, return_y=False)
    x_val, s_val, _ = checkmark_dataset(n_samples, seed*5, 0.1, return_y=False)
    x_test, s_test, _ = checkmark_dataset(n_samples, seed*10, 0.9, return_y=False)
    c_train = s_train[:, [0, 1, 2]].clone()
    y_train = s_train[:, 3]
    c_test = s_test[:, [0, 1, 2]].clone()
    y_test = s_test[:, 3]
    c_val = s_val[:, [0, 1, 2]].clone()
    y_val = s_val[:, 3]
    ...
    models = {
        'CausalHCEM_0_0.05_det': CausalHCEM(x_train.shape[1], embedding_size, n_concepts, n_classes, ce_size, g, 0, lambda_cace_2, probabilistic=False),
        'CausalHCEM_0_0.05_det_given': CausalHCEM(x_train.shape[1], embedding_size, n_concepts, n_classes, ce_size, g, 0, lambda_cace_2, probabilistic=False),
    ...
    for model_name, model in models.items():
        ...
        if model_name.endswith('given'):
            ...
            cov = torch.abs(torch.tensor(np.corrcoef(s_train.T))).float()/10
            with torch.no_grad():
                model.concept_embedder.eq_model.fc1.weight = torch.nn.Parameter(cov, requires_grad=True)
        # try:
        trainer = Trainer(max_epochs=350, accelerator='cpu', enable_checkpointing=True, callbacks=checkpoint_callback)
        ...
        if model.__class__.__name__ in ['CausalCEM', 'CausalCBM', 'CausalCBMDAGMA', 'CausalCEMDAGMA', 'CausalHCEM']:
            ...
            plt.tight_layout()
            plt.savefig(os.path.join(results_dir, f'checkmark_heatmap_{model_name}_{seed}.pdf'))
            # plt.show()
            ...
            acc_int, abs_acc_int = interventions_from_root(torch.tensor(dag), model, x_test_shuffled, s_test, exclude=[n_concepts])
            print(acc_int)
            ...
            diff_c_mean = diff_c.mean(dim=0)
            order_diff = torch.argsort(diff_c_mean, descending=True)
            acc_int, abs_acc_int = interventions_from_root(torch.tensor(dag_init), model, x_test_shuffled, s_test, order_diff, exclude=[n_concepts])
            print(acc_int)
        ...
        if model_name == 'BB':
            ...
            # save results
            results.append(['checkmark_dataset', model_name, seed, s_accuracy])
            ...
            print(f'{model_name} label accuracy: {s_accuracy}')
            concept_accuracy = accuracy_score(c_test.ravel(), s_pred[:, :-1].ravel() > 0.5)
            print(f'{model_name} concept accuracy: {concept_accuracy}')
            task_accuracy = accuracy_score(y_test, s_pred[:, -1] > 0.5)
            print(f'{model_name} task accuracy: {task_accuracy}')
            ...
            # save results
            results.append(['checkmark_dataset', model_name, seed, s_accuracy, average_cace, min_cace, max_cace, fidelity, label_accuracy_perturb, label_accuracy_perturb_block, cace, cace_block])
            ...
            interventions.append(['checkmark_dataset', model_name, seed, acc_int])
            metric_cols = ['acc_int']
            ...
            abs_interventions.append(['checkmark_dataset', model_name, seed, abs_acc_int])
            metric_cols = ['abs_acc_int']
            df = pd.DataFrame(interventions, columns=['dataset', 'model', 'seed'] + metric_cols)
            df.to_csv(os.path.join(results_dir, 'abs_interventions.csv'), index=False)


            pns_list.append(['checkmark_dataset', model_name, seed, PNS])
            metric_cols = ['PNS']
